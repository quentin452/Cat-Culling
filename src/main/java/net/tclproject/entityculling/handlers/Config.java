...
public class Config {
  ...
  public static void load(FMLPreInitializationEvent event) {
    ...
    tracingDistance = config.getInt(
        "tracingDistance", GENERIC_CATEGORY, 64, Short.MIN_VALUE,
        Short.MAX_VALUE,
        "128 works out to be roughly equal to minecraft's defaults");
    sleepDelay = config.getInt("sleepDelay", GENERIC_CATEGORY, 10,
                               Short.MIN_VALUE, Short.MAX_VALUE,
                               "The delay between async pathtracing runs " +
                               "that update which TEs need to be culled");
    hitboxLimit = config.getInt("hitboxLimit", GENERIC_CATEGORY, 50,
                                Short.MIN_VALUE, Short.MAX_VALUE,
    ...
    String blockEntityWhitelistString =
        config
            .get(GENERIC_CATEGORY, "entityWhitelist", "tile.beacon",
    ...
    renderNametagsThroughWalls =
        config.getBoolean("renderNametagsThroughWalls", GENERIC_CATEGORY, true,
                          "Renders nametags through walls");
    debugMode = config.getBoolean("debugMode", GENERIC_CATEGORY, false,
                                  "Try this before sending an issue report.");
    aggressiveMode = config.getBoolean(
        "aggressiveMode", GENERIC_CATEGORY, false,
