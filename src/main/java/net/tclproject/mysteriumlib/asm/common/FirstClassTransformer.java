...
/**
 * This transformer transforms any classes that load before minecraft's classes do. When minecraft
 * classes start loading (actually, a bit earlier - in Loader.injectData) all fixes from this class
 * are transferred to CustomClassTransformer. This is needed in order for the fixes in MysteriumASM
 * Lib to be applied after forge deobfuscation.
 */
public class FirstClassTransformer extends TargetClassTransformer implements IClassTransformer {
  ...
  public FirstClassTransformer() {
    ...
    if (instance != null) {
      // If the lib has been loaded, clear all fixes that for some reason have been put in and
      // transfer them into this instance.
  ...
  /**
   * Forge passes in all the classes here to be transformed. We are passing them to the actual
   * transformer with the right arguments.
   */
  @Override
   ...
   * logic to see if the method is the target method, accounting for an obfuscated descriptor.
   */
  @Override
