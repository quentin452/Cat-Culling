...
public class CustomLoadingPlugin implements IFMLLoadingPlugin {
  ...
  /**
   * Returns the transformer that we are using at the current moment in time to modify classes. See
   * why we have to use two separate ones in the documentation for FirstClassTransformer.
   *
   * @return FirstClassTransformer if our built-in fixes haven't been applied, otherwise -
  ...
  /**
   * Registers a single manually made ASMFix. It is not the most efficient way to make fixes, but if
   * you want to go this way, look at how the code already there builds an ASMFix out of a fix
   * method or just take a look at the documentation of the builder class within ASMFix.
   */
  ...
  /**
   * If the obfuscation has not yet been checked, checks and returns it. If it has, returns the
   * value that the previous check returned.
   *
  ...
  public void injectData(Map<String, Object> data) {
    ...
    // Some mods may refer to others in their name, so we'll to confirm those assumption with class
    // presence check.
