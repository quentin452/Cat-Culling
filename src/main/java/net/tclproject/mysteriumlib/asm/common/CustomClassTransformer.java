...
/**
 * This class is responsible for inserting fixes into minecraft code from the moment when forge
 * deobfuscation was applied.
 */
public class CustomClassTransformer extends TargetClassTransformer implements IClassTransformer {
  ...
  public CustomClassTransformer() {
    ...
    if (CustomLoadingPlugin
        .isObfuscated()) { // If running in obf environment, load obf method names
      try {
    ...
    this.fixesMap.putAll(
        FirstClassTransformer.instance
            .getFixesMap()); // Puts all fixes loaded in FirstClassTransformer into this class.
  ...
  /**
   * Inserts fixes into classes passed in. First, runs the class through the normal transformer, and
   * only then, through the post transformers (ones that are to be executed after the normal ones).
   *
   * <p>Forge passes in the arguments and takes the return value of this method, unless you want
   * some special behavior you shouldn't interact with this method or modify it.
   */
  @Override
  ...
  /**
   * Creates a custom Class Visitor to return custom method visitors to insert fixes. Has custom
   * logic to check if a method is the target method of a fix, accounting for an obfuscated name.
   */
  @Override
  ...
  /**
   * Registers a transformer that will be executed after the normal transformers (including the
