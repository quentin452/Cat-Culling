...
/**
 * An add-on to MetaReader for getting information related to minecraft obfuscation and
 * deobfuscation.
 */
public class MinecraftMetaReader extends MetaReader {
   ...
   *
   * <p>NOTE: It does not check the mcp vs srg descriptors, it only checks if they are the same
   * which in most cases will not be the case if it's srg vs mcp.
   *
  ...
  /**
   * Forge and others can create methods at runtime. This method gets a method reference, even if
   * the target method is runtime-generated by other mods or forge.
   *
  ...
  /**
   * Gets the modified class from a deobfuscated class name. The modified class is the class with
   * all the fixes, mixins, asm etc. applied to it from everywhere.
   *
  ...
  /**
   * Returns an obfuscated name from a deobfuscated one if run in an obfuscated environment. If run
   * in a deobfuscated environment, the name stays the same.
   *
