 ...
 * depends in what part of the code the fix will be inserted. By deault, there are two types of
 * inserters: OnEnter, which inserts the fix to the start of the method, and OnExit, which inserts
 * it on every exit.
 */
public abstract class FixInserterFactory {

    /**
     * The method AdviceAdapter#visitInsn() is a weird thing. For some reason, the calling of the next
     * MethodVisitor is done after the logic and not before, like in all the other cases. That's why
     * for MethodExit the priority of fixes is the reverse.
     */
