...
/**
 * The main ASM class, responsible for the insertion of a fix into a method. PLEASE DO NOT MODIFY
 * ANY OF THE FIELDS INSIDE MANUALLY, USE THE BUILDER AND IT'S SETTERS OR YOU RISK BREAKING THINGS.
 *
 * <p>--*--
 *
 * <p>Terms used:
 *
 * <p>Fix - the execution of your static code from minecraft's, forge's, or another mod's code
 *
 * <p>Target Method - method, inside which you put your fix
 *
 * <p>Target Class - the class that has the method inside which you put your fix
 *
 * <p>Fix Method - your static method that gets called from other's code
 *
 * <p>Fix Class (Class with fixes) - the class, inside which your fix method/s is located.
 */
public class ASMFix implements Cloneable, Comparable<ASMFix> {
  ...
  /**
   * The return method's name. The return method is the method the return value of which is passed
   * if returnType is ANOTHER_METHOD_RETURN_VALUE.
   */
  public String returnMethodName;
  /**
   * The return method's descriptor. Can be without the return type. The return method is the method
   * the return value of which is passed if returnType is ANOTHER_METHOD_RETURN_VALUE.
   */
  ...
  public boolean isTheTarget(String name, String descriptor) {
    return (targetMethodReturnType == null && descriptor.startsWith(targetMethodDescriptor)
            || descriptor.equals(targetMethodDescriptor))
  ...
  /**
   * Returns if a fix method is stored inside this ASMFix (in other words, if fixMethodName != null
   * && classWithFixes != null).
   */
  ...
  public void createMethod(FixInserterClassVisitor classVisitor) {
    MetaReader.MethodReference superMethod =
        classVisitor.transformer.metaReader.findMethod(
            getTargetClassInternalName(), targetMethodName, targetMethodDescriptor);
  ...
  /**
   * Inserts a load instruction for the type passed. A load instruction loads a variable from the
   * local variable table onto the stack.
   *
  ...
  public void insertLoad(FixInserter inserter, Type parameterType, int variableIndex) {
    int opcode;
    if (parameterType == INT_TYPE
        || parameterType == BYTE_TYPE
  ...
  public void insertSuperCall(FixInserter inserter, MetaReader.MethodReference method) {
    ...
    for (int i = 0; i <= targetMethodArguments.size(); i++) {
      Type argumentType =
          i == 0 ? TypeUtils.getType(targetClassName) : targetMethodArguments.get(i - 1);
      insertLoad(inserter, argumentType, variableIndex);
      if (argumentType.getSort() == Type.DOUBLE
          || argumentType.getSort()
              == Type
                  .LONG) { // if it is a long or a double, it occupies two variables, therefore we
        // need to insert the load for the one two later, skipping one
        variableIndex += 2;
      } else {
        variableIndex++;
      }
  ...
  /**
   * Inserts an instruction to push the target method return type's default value onto the stack,
   * e.g. int -> 0, object -> null
   *
   * @param inserter Inserter that is inserting this fix.
  ...
  public void insertReturn(FixInserter inserter, Type targetMethodReturnType) {
    if (targetMethodReturnType == INT_TYPE
        || targetMethodReturnType == SHORT_TYPE
  ...
  /**
   * Inserts an instruction to call a static method. This instruction is used to insert a call to
   * the fix methods inside the target methods.
   *
   * @param inserter Inserter that is inserting this fix.
  ...
  public void insertInvokeStatic(
    ...
    for (int i = 0;
        i < fixMethodArguments.size();
        i++) { // Iterate through all the fix method's arguments
    ...
    inserter.visitMethodInsn(
        INVOKESTATIC,
        getClassWithFixesInternalName(),
        name,
        descriptor,
        false); // instruction, owner class, method name, method descriptor, if the owner class is
    // an interface
  }
  ...
  public String toString() {
    ...
    if (EnumReturnType == EnumReturnType.PRIMITIVE_CONSTANT)
  ...
  // also has good java docs, this one just compares the order at which the fixes will be inserted.
  // 1: the priority is greater (HIGHEST equiv.), -1: less (LOWEST equiv.)
  @Override
  public int compareTo(ASMFix fix) {
    if (injectorFactory.priorityReversed
        && fix.injectorFactory.priorityReversed) { // if both are inserted at the end
      return priority.ordinal() > fix.priority.ordinal()
          ? -1
          : 1; // Since priority is reversed, if our priority is greater (>) it is actually less
      // (-1)
    } else if (!injectorFactory.priorityReversed
        && !fix.injectorFactory.priorityReversed) { // if both are inserted at the beginning
      return priority.ordinal() > fix.priority.ordinal()
          ? 1
          : -1; // If our priority is greater (>), it's greater (1)
    } else {
      return injectorFactory.priorityReversed
          ? 1
          : -1; // If both are inserted at different places: If we are inserted at the end, we have
      // greater priority.
    }
  }
  ...
  public class Builder extends ASMFix {
     ...
     *
     * <p>Defines the name of the class, into which the fix will be inserted.
     *
     ...
     * -----[NECESSARY TO CALL THIS FOR EVERY BUILDER]-----
     *
     * <p>Defines the name of the method, into which the fix will be inserted. If fixing a
     * constructor is needed, the method name is <init>.
     *
     ...
     *
     * <p>Adds one or more arguments to the list of arguments of the target method.
     *
     * <p>Those arguments are used to create the descriptor of the target method. In order to
     * precisely find the target method, the name by itself is not enough, as methods with the same
     * name but different arguments exist.
     *
     * <p>Example of use:
     *
     * <p>import static net.tclproject.mysteriumlib.asm.core.TypeUtils.*
     *
     * <p>//...
     *
     * <p>addTargetMethodParameters(Type.INT_TYPE)
     *
     * <p>Type worldType = getType("net.minecraft.world.World")
     *
     * <p>Type playerType = getType("net.minecraft.entity.player.EntityPlayer")
     *
     * <p>addTargetMethodParameters(worldType, playerType, playerType)
     *
    ...
    /**
     * Adds one or more arguments to the list of arguments of the target method. This is an
     * abstraction layer for addTargetMethodParameters(Type... parameterTypes), that resolves the
     * types from names.
     *
     * <p>
     *
    ...
    /**
     * Specifies the return type of the target method. The return type is used to create the
     * descriptor of the target method. In order to precisely find the target method, the name by
     * itself is not enough, the descriptor is also needed. By default, the fix inserts into all
     * methods with the right name and arguments.
     *
    ...
    /**
     * Specifies the return type of the target method. This is an abstraction layer for
     * setTargetMethodReturnType(Type returnType), that resolves the type from it's name.
     *
     ...
     *
     * <p>Specifies the name of the class with the fix method.
     *
     ...
     * -----[NECESSARY TO CALL THIS IF A FULL FIX IS NEEDED (NOT ONLY AlwaysReturned)]-----
     *
     * <p>Specifies the name of the fix method. The fix method MUST be static, and there is no check
     * to verify that. Be careful, please.
     *
     ...
     *
     * <p>Adds an argument to the list of arguments of the fix method. In bytecode, the names of the
     * arguments aren't saved. Instead, the use of indexes is needed.
     *
     * <p>For example, in the class EntityLivingBase there is a method:
     * attackEntityFrom(DamageSource damageSource, float damage). Inside it, these indexes will be
     * used:
     *
     * <p>1 - damageSource 2 - damage
     *
     * <p>IMPORTANT: longs and doubles take up two spaces. Theoretically, not only the original
     * aguments can be passed in to the fix method but also local variables, but their indexes will
     * be way harder to calculate.
     *
     * <p>For example, in the class Entity there is a method: setPosition(double x, double y, double
     * z). Inside it, these indexes will be used:
     *
     * <p>1 - x 2 - пропущено 3 - y 4 - пропущено 5 - z 6 - пропущено
     *
     * <p>The code of this method:
     *
     * <p>//... float f = ...; float f1 = ...; //...
     *
     * <p>In this case, the index of f will be 7, and f1: 8.
     *
     * <p>If the target method is static, you don't start counting the indexes from 0, The numbers
    ...
    /**
     * Adds an argument to the list of arguments of the fix method. This is an abstraction layer for
     * addFixMethodParameter(Type parameterType, int variableIndex), that resolves the type from
     * it's name.
     *
    ...
    /**
     * Adds the target class to the fix method's list of arguments to later be passed this. If the
     * target method is static, null will be passed.
     *
    ...
    /**
     * Adds the return type of the target method to the fix method's list of arguments to later be
     * passed the returned value. In other words, it's the top value on the stack when the fix
     * method is called.
     *
     * <p>e.g. this is the code of a method:
     *
     * <p>int foo = bar();
     *
     * <p>return foo;
     *
     * <p>Or this is the code of the method:
     *
     * <p>return bar()
     *
     * <p>In both cases we can pass the fix method the returned value before return is called,
     * because of how bytecode works.
     *
    ...
    /**
     * Specifies a setting, in which, return is called after the fix method has been. By default,
     * return isn't called at all.
     *
     * <p>Besides that, this method changes the return type of the fix method:
     *
     * <p>NEVER -> void ALWAYS -> void ON_TRUE -> boolean ON_NULL -> Object ON_NOT_NULL -> Object
     *
     * @param setting The condition, if which, it will exit from the target method after the fix
     *     method has been called.
     * @throws IllegalArgumentException if setting == ON_TRUE, ON_NULL or ON_NOT_NULL, but the fix
    ...
    /**
     * -----[NECESSARY TO CALL THIS IF THE TARGET METHOD DOESN'T RETURN void AND setReturnSetting
     * HAS BEEN CALLED]-----
     *
     * <p>Specifies the type that gets returned after the fix method has been called. Call this
     * after setReturnSetting. By default, void is returned.
     *
     * <p>Besides that, if value == EnumReturnType.FIX_RETURN_VALUE, this method changes the return
     * type of the fix method to the type, specified in setTargetMethodReturnType()
     *
     * @param type the return type, e.g. EnumReturnType.FIX_RETURN_VALUE, PRIMITIVE_CONSTANT etc.
     * @throws IllegalStateException if EnumReturnSetting == NEVER it doesn't make sense to specify
     *     the return type, since return doesn't get called.
     * @throws IllegalArgumentException if value == EnumReturnType.FIX_RETURN_VALUE, but the return
     *     type of the target method is void (or setTargetMethodReturnType hasn't been called). It
     *     doesn't make sense to use the value that the fix method returned, if it returned void (it
     *     must match the type returned by the target method).
     * @return this
     */
    public Builder setReturnType(EnumReturnType type) {
      ...
      if (type == EnumReturnType.PRIMITIVE_CONSTANT
          && returnType != null
      ...
      if (type
          == EnumReturnType
              .FIX_METHOD_RETURN_VALUE) { // If the return type is the type of the fix method, we
        // set the return type of the fix method to one of the
        // target method.
        ASMFix.this.fixMethodReturnType =
            ASMFix.this
                .targetMethodReturnType; // That is because the type must fit to be returned from
        // the target method.
      }
      return this;
    }
    ...
    /**
     * -----[NECESSARY TO CALL THIS IF setReturnType HAS BEEN CALLED AND SET TO
     * PRIMITIVE_CONSTANT]-----
     *
     * <p>Call this after setReturnType(EnumReturnType.PRIMITIVE_CONSTANT). Specifies the primitive,
     ...
     * @throws IllegalArgumentException If the class of the passed in value isn't a wrapper for a
     *     primitive
     * @return this
     */
    ...
    /**
     * -----[NECESSARY TO CALL THIS IF setReturnType HAS BEEN CALLED AND SET TO
     * ANOTHER_METHOD_RETURN_METHOD]-----
     *
     * <p>Call this after setReturnType(EnumReturnType.ANOTHER_METHOD_RETURN_VALUE). Specifies the
     * method, the return value of which will be returned.
     *
    ...
    /**
     * Sets the factory that will specify the inserter type. In other words, specifies, if the fix
     * will be inserted at the start (ASMFix.ON_ENTER_FACTORY) or end (ON_EXIT_FACTORY) of a method.
     * If you need OnLine, create your own factory.
     *
    ...
    /**
     * Sets the priority of inserting this fix. Fixes with higher priority will be inserted first.
     *
     * @return this
     */
    ...
    /**
     * Setter for createMethod. NOTE: The method body is a call to super() if it's an override, or
     * only the return statement otherwise.
     *
     * @return this
     */
    ...
    public ASMFix build() {
      ...
      if (fix.createMethod && fix.targetMethodReturnType == null) {
        fix.targetMethodReturnType =
            fix.fixMethodReturnType; // If we're to create a method, it's return type is the one of
        // the fix method
      }
      fix.targetMethodDescriptor =
          getMethodDescriptor(
              fix.targetMethodReturnType,
              fix.targetMethodArguments); // Creates the target method descriptor out of the
      // arguments and return type

      if (fix.hasFixMethod()) {
        fix.fixMethodDescriptor =
            Type.getMethodDescriptor(
                fix.fixMethodReturnType,
                fix.fixMethodArguments.toArray(
                    new Type
                        [0])); // If we have a full fix method specified, we create it's descriptor
        // out of it's return type and arguments
      }
      if (fix.EnumReturnType == EnumReturnType.ANOTHER_METHOD_RETURN_VALUE) {
        fix.returnMethodDescriptor =
            getMethodDescriptor(
                fix.targetMethodReturnType,
                fix.fixMethodArguments); // If we're to return what a third method returns, we
        // create it's desctiptor out of it's return type and
        // arguments
      }
      ...
      if (fix.EnumReturnType == EnumReturnType.PRIMITIVE_CONSTANT
