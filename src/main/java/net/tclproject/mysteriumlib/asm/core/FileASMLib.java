...
/**
 * Class that enables Mysterium ASMLib to work outside of minecraft. It has a main method that must
 * be executed first.
 *
 * <p>IMPORTANT: This is not a ready-to-go class, for it to work, you need to implement some form of
 * interaction with the modified classes, but that is up to you.
 */
public class FileASMLib {
  ...
  /**
   * Registers classes with fix methods, and runs the transformer through the original classes to
   * insert the fixes.
   */
  void transform() throws IOException {
    ...
    for (File file : getFiles(".class", originalClasses)) {
      ...
      String className =
          ""; // we need to get the class name e.g. com.example.ExampleMod out of a path
      byte[] newBytes = transformer.transform(className, bytes);

      // IMPORTANT: Now you need to do something with newBytes[]. Perhaps, look at how forge loads
      // modified minecraft classes.
    }
  }

  /**
   * Gets a list of files in a directory and all it's subdirectories that end with a specific
   * string.
   *
