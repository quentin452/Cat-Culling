...
/**
 * Class that processes byte[] classes, to be integrated with IFMLLoadingPlugin to get classes
 * passed into transform method.
 */
public class TargetClassTransformer {
  ...
  public void registerFix(ASMFix fix) {
    if (fixesMap.containsKey(fix.getTargetClassName())) {
      ...
      List<ASMFix> list =
          new ArrayList<ASMFix>(2); // Create a new list of fixes to be applied to the class.
      list.add(fix); // Add this fix to the list.
      fixesMap.put(
          fix.getTargetClassName(),
          list); // Put the class and the list of fixes for it into the map.
    }
  }
  ...
  /**
   * Takes the original bytecode of a class and returns the modified version of it with fixes
   * applied.
   */
  public byte[] transform(String className, byte[] classBytes) {
    ...
    if (fixes != null) { // if there are any
        ...
        */
        ...
        ClassWriter classWriter =
            createClassWriter(
                java7
                    ...
                    : ClassWriter
                        .COMPUTE_MAXS); // If java is 7+, compute frames, if not, set everything to
        // max possible
        ...
        classReader.accept(
            ...
            java7
                ...
                : ClassReader
                    .EXPAND_FRAMES); // Make the fix inserter class visitor run through the methods
        // and return fix inserter method visitors

        // Chain: register class with fix methods -> parse the class for fix methods and add them to
        // the list to be inserted -> make custom class visitor -> visit target class and return
        // custom method visitors that will be executed -> custom method visitors call ASMFix method
        // to insert fixes from the list -> ASMFix inserts the fixes

        classBytes =
            classWriter.toByteArray(); // Overwrite the class bytes with the new class bytes
        ...
        fixes.removeAll(
            fixInserterVisitor
                .insertedFixes); // remove inserted fixes from the list of fixes to be inserted
      ...
      for (ASMFix notInserted :
          fixes) { // since inserted fixes get removed, we can just iterate through ones left
  ...
  /**
   * Creates a custom Class Visitor to return custom method visitors to insert fixes. This method
   * can be overridden, if inside the ClassVisitor, custom logic is needed to check if the methods
   * are the target methods.
   *
  ...
  /**
   * Creates a Class Writer for storing the transformed class. This method can be overridden, if a
   * custom implementation of getCommonSuperClass is needed. The standard implementation works only
   * for classes, the .class files of which are already inside classpath, but aren't loaded. The
   * loading (but not init) of classes occurs. If the loading of classes is a problem, you can use
   * SafeCommonSuperClassWriter.
   *
