...
public class MiscUtils {
  ...
  /**
   * ClassWriter with a custom implementation of getCommonSuperClass. When it's used, no class
   * loading occurs. But, the loading of classes itself is rarely the problem, because the init of
   * the class (the calling of static blocks) doesn't happen when class loading occurs. The problems
   * occur, when fixes are inserted into classes dependent on each other, then the standard
   * implementation fails with ClassCircularityError.
   */
    ...
    protected String getCommonSuperClass(String type1, String type2) {
      ...
      int size =
          Math.min(
              superClasses1.size(),
              superClasses2
                  .size()); // the maximum time both can be super'd until one reaches one of classes
      // passed in
      int i;
      ...
      if (i == 0) {
        return "java/lang/Object"; // If there are no common classes, Object is automatically
        // returned
      } else {
        return superClasses1.get(
            i - 1); // e.g: you have 1 common class so "i" is going to be 1. But list indices
        // start with 0, that's why it takes away 1.
      } // The common class returned is the most specific one, e.g. if we have 'class a extends b
      // extends c', b is returned
    }
  }
  ...
  public interface LogHelper {
    ...
    /**
     * Used for general information. Does not appear in the game console, in order to see it, one
     * must open up fml-server/client-latest.log.
     */
   ...
   * methods.csv is located inside mcp/conf/. (mcp is the folder you unzipped mcp into). If you
   * didn't know, forge uses mcp to provide you with readable method names. Inside this folder,
   * there are ready-to-go methods.bin files for 1.6.4, 1.7.10, and 1.8.
   *
   * <p>It is still highly recommended to generate methods.bin for your own version of mcp,
   * otherwise errors like "can't find target method of fix" will most likely occur at some point.
   *
   * @throws Exception if something went wrong, e.g. the program doesn't have the permissions to
   *     access the file or folder it's in.
   */
